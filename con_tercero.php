...
if($_REQUEST['eliminacion']==1) {//confirmacion de eliminacion 
	$campos="estado_ter ='2'";
	$error=editar("tercero",$campos,'cod_ter',$eli_codigo); 
	if ($error >=1)
...
if(!empty($_REQUEST['busquedas'])) {
  if($_REQUEST['campo'] == 'nombre'){
    ...
    $where="having $busquedas and estado_ter = 1";
  } 
  else{
    $busquedas=reemplazar_1($busquedas);
    $where="where $busquedas and estado_ter = 1";
  }
}
else{
	$where="where estado_ter = 1";
}

$sql="SELECT *,CONCAT(nom1_ter,' ',nom2_ter,' ',apel1_ter,' ',apel2_ter) as nombre FROM tercero 
INNER JOIN tipo_identificacion ON tipo_identificacion.cod_tiden = tercero.tiden_ter
                    ...
                    <tr>
                      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_tercero.php?codigo=0&editar=<?php echo$editar?>&insertar=<?php echo$insertar?>&eliminar=<?php echo$eliminar?>'"/>
					  	  <?php } ?></td>
                    ...
                    <tr>
                      <td><table width="100%" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							echo "<td >$db->id_ter</td>";
							echo "<td >$db->nom_cat</td>";
                            echo "<td aling='center' >"; 
							
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_ter'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_ter.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ter) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.png' width='16' height='16'  /></td> ";

               //impresion  
              echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('carnet.php',$db->cod_ter,'grande')\" /></td>"; 
